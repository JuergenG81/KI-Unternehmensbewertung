...
revenue_growth = st.sidebar.slider("Umsatzwachstum (p.a.)", -0.50, 1.00, 0.10, step=0.01)
ebitda_margin = st.sidebar.slider("EBITDA-Marge", 0.00, 0.60, 0.18, step=0.01)
depr_ratio = st.sidebar.slider("Abschreibungen / Umsatz", 0.00, 0.20, 0.03, step=0.005)
tax_rate = st.sidebar.slider("Steuersatz", 0.00, 0.60, 0.25, step=0.01)
nwc_ratio = st.sidebar.slider("ΔNWC / Umsatz", -0.20, 0.20, 0.02, step=0.005)
capex_ratio = st.sidebar.slider("CAPEX / Umsatz", 0.00, 0.30, 0.04, step=0.005)
...
def recompute_equity(wacc_, growth_, ebitda_margin_):
    ...
    for t in range(1, int(forecast_years) + 1):
        revenue = revenue * (1 + revenue_growth)
        r = fcff_from_drivers(
            revenue, ebitda_margin_, depr_ratio, tax_rate, nwc_ratio, capex_ratio
        )
        r["year"] = t
    ...
    last_fcff_ = df.iloc[-1]["fcff"]
    tv_ = gordon_terminal_value(last_fcff_, wacc_, growth_)
    pv_tv_ = tv_ * discount_factor(wacc_, int(forecast_years))
    ev_ = df["pv_fcff"].sum() + pv_tv_
    return equity_from_ev(ev_, debt, cash)

eb_low = recompute_equity(wacc, terminal_growth, max(0.0, ebitda_margin - 0.10))
eb_high = recompute_equity(wacc, terminal_growth, min(0.60, ebitda_margin + 0.10))
...
report_lines.append(
    f"<p>E (nachhaltiger Ertrag, vor St.): {E:,.0f} €<br>"
...
if substanzwert is not None and not np.isnan(wien_value):
    mw = (wien_value + substanzwert) / 2.0
    report_lines.append(
        f"<p>Plausibilisierung (Mittelwert mit Substanzwert {substanzwert:,.0f} €): {mw:,.0f} €</p>"
    )
